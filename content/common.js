...
function isUnsupportedPACERHost(hostname) {
    return (UNSUPPORTED_PACER_DOMAINS.indexOf(hostname) >= 0);
}
...
function havePACERCookie() {
    ...
    while (cookieEnum.hasMoreElements()) {
        ...
        if (cookie instanceof Components.interfaces.nsICookie && cookie.host.match("uscourts.gov")) {
...
function hasECFCookie() {
    ...
    while (cookieEnum.hasMoreElements()) {
        ...
        if (cookie instanceof Components.interfaces.nsICookie &&
            cookie.host.match("uscourts.gov") && cookie.name.match("KEY"))
            return true;
    }
